function [X, info] = tucker_sssd_pc(A, C, M, options)
%--------------------------------------------------------------------------
% Tucker subspace Steepest Descent method with preconditioning for 
...
%               iteration, i.e., last computed coefficient alpha, 'no beta', 
...
if type_prec == 0 % No Preconditioning
    P = Cfact;
elseif type_prec == 1 % Preconditioning using exponential sum
    if M.fftprec % Precinditioner built using FFT
        disp('Prec FFT')
    ...
    else % Preconditioned built using Eigs
...
while (rel_res > tol)  && (it <= maxit)
    ...
    elseif innsolver == 1 % PCG
        ...
        prodPtAP = @(x) tucker_Ax_trunc(PtAP, x, maxrank, 1e-12, single_flag);
    ...
    % Updating P
    if type_prec == 0 % No Preconditioner
         P = cell(1,d);
        sz_p = zeros(1,d);
        for k=1:d
            P{k} = Res_fact{k};
...
info = {alpha, 'no beta'; Res, P; it, resvec};
