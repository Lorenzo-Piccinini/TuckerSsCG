function [Ycore, Yfact] = tucker_Mx_trunc_FFTexp(M, Xcore, Xfact, maxrank, tol_tr)
%--------------------------------------------------------------------------   
...
hat_Xfact = cell(1,d);
for k = 1:d
    hat_Xfact{k} = my_dst((Xfact{k})); % FFT-based fast transform on columns
    Xfact{k} = [];
end
for j = 1:nterms
    for k=1:d
        idx = (j-1)*r(k) + 1 : j*r(k);
        tmpV{k}(:, idx) = hat_Xfact{k}.*(diag_filter{j});
    end
end
tmpQ = cell(1,d);
R = cell(1,d);
...
Yfact = cell(1,d);
for k = 1:d
    Yfact{k} = (my_idst(tmpQ{k}*newYfact{k}));
    tmpQ{k} = [];
    newYfact{k} = [];
end
Ycore = (Ycore);
